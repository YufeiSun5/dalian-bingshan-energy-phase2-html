...
class ChartDataManager {
    ...
    generateSQL() {
        ...
        const sql = `
            ...
            SELECT
                ...
                FORMAT(AVG(CASE WHEN d.COD IS NOT NULL THEN d.COD ELSE NULL END), 2) AS cod_avg
            FROM
                time_slots t
            LEFT JOIN
                data_foul_water d
    ...
    async fetchData() {
        try {
            ...
            console.log('%c[SQL查询]', "color: #00b894; padding: 2px 5px;", "5天分时段-COD统计");
            ...
            const values = response.data.map(item => item.cod_avg);    // COD数值 (可能为 null)
            ...
            const transformedData = {
                ...
                series: [{
                    name: 'COD',
                    values: values,
                    color: '#ff8c00', // 亮橙色，与其他水质监控系统保持一致
                    unit: 'mg/L'      // COD常用单位
                }]
            };
        ...
        for (let i = 0; i < this.config.retryTimes; i++) {
            try {
                ...
                if (window.updateChart) {
                    ...
                    console.log(`%c[数据更新] COD`, "color: #00b894; padding: 2px 5px;");
                }
                break;
            } catch (error) {
...
window.addEventListener('DOMContentLoaded', function() {
    console.log('%c[初始化] COD监控系统...', "color: #00b894; font-weight: bold;");
    ...
    setTimeout(() => {
        ...
        //             { value: 100, name: '排放限值', color: '#ff7675', width: 2, type: 'dashed', unit: 'mg/L' },
        //             { value: 60, name: '警戒线', color: '#ffeaa7', width: 1, type: 'solid', unit: 'mg/L' }
        //         ]);
        //     }
        // }, 500);
...
console.log("%c 🧪 污水处理监控系统 - COD", "color: #00d2d3; font-weight: bold; font-size: 16px;");
console.log("%c━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━", "color: #666;");
console.log(`
%c可用命令：
