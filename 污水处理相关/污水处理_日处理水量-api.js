...
class ChartDataManager {
    ...
    generateSQL() {
        const sql = `
            SELECT DATE_FORMAT(DATE_ADD(DATE_FORMAT(NOW(), '%Y-%m-01'), INTERVAL days.n DAY),'%m-%d')  AS day,sum(r.today_water) as today_water
    ...
    async fetchData() {
        try {
            ...
            const values = response.data.map(item => item.today_water);
