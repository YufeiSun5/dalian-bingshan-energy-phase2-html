...
class ChartDataManager {
    ...
    generateSQL() {
        const sql = `
            SELECT DATE_FORMAT(DATE_ADD(DATE_FORMAT(NOW(), '%Y-%m-01'), INTERVAL days.n DAY),'%m-%d')  AS day,sum(r.today_elec) as today_elec																																			
			FROM (																																			
			...
			left join (																																			
					...
					group by DATE_FORMAT(date, '%Y-%m-%d %H:00'),day																																	
			) r on DATE_FORMAT(DATE_ADD(DATE_FORMAT(NOW(), '%Y-%m-01'), INTERVAL days.n DAY),'%d')  = day																																			
    ...
    async fetchData() {
        try {
            ...
            const values = response.data.map(item => item.today_elec);
        ...
        /*setTimeout(() => {
            if (window.chartControl && window.chartControl.setMarkLine) {
                window.chartControl.setMarkLine([
                    { value: 105, name: '预警值', color: '#ff0000', width: 2, type: 'dashed', unit: 'kW·h' }
                ]);
            }
        }, 500);*/
