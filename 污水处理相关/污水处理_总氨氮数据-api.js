...
class ChartDataManager {
    ...
    generateSQL() {
        ...
        const sql = `
            ...
            SELECT
                ...
                FORMAT(AVG(CASE WHEN d.NH3 IS NOT NULL THEN d.NH3 ELSE NULL END), 2) AS nh3_avg
            FROM
                time_slots t
            LEFT JOIN
                data_foul_water d
    ...
    async fetchData() {
        try {
            ...
            console.log('%c[SQL查询]', "color: #00b894; padding: 2px 5px;", "5天分时段-氨氮统计");
            ...
            const values = response.data.map(item => item.nh3_avg);    // 氨氮数值 (可能为 null)
        ...
        for (let i = 0; i < this.config.retryTimes; i++) {
            try {
                ...
                if (window.updateChart) {
                    ...
                    console.log(`%c[数据更新] NH3`, "color: #00b894; padding: 2px 5px;");
                }
                break;
            } catch (error) {
...
window.addEventListener('DOMContentLoaded', function() {
    console.log('%c[初始化] 污水处理监控系统...', "color: #00b894; font-weight: bold;");
...
console.log("%c 💧 污水处理监控系统", "color: #00d2d3; font-weight: bold; font-size: 16px;");
console.log("%c━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━", "color: #666;");
console.log(`
%c可用命令：
...
• 预警值: 排放限值 5.0 mg/L（红色虚线）
• 预警值: 警戒线 2.0 mg/L（黄色实线）
