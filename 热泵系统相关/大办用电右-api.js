...
class TotalPowerFinal {
        ...
        const sql = `
            SELECT
                DATE_FORMAT(STR_TO_DATE('07:00:00', '%T') + INTERVAL t.i HOUR, '%H:00') AS time_label,
								...
								ROUND((sum_heat5 * 1000)/(elec.sum_elec5*3.6),2) as sum_rated5,
								ROUND((sum_heat6 * 1000)/(elec.sum_elec6*3.6),2) as sum_rated6
            FROM sys_ints t
            ...
            LEFT JOIN (
                 select 	DATE_FORMAT(r.jDay, '%H') as date,  
									...
									sum(case when elec_name in ('elec_420001') then sum_elec else 0 end) as sum_elec1,   
									sum(case when elec_name in ('elec_420011') then sum_elec else 0 end) as sum_elec2,
									sum(case when elec_name in ('elec_420002') then sum_elec else 0 end) as sum_elec3,
									sum(case when elec_name in ('elec_420009') then sum_elec else 0 end) as sum_elec4,
									sum(case when elec_name in ('elec_320022') then sum_elec else 0 end) - sum(case when elec_name in ('elec_420002') then sum_elec else 0 end) - sum(case when elec_name in ('elec_420001') then sum_elec else 0 end) - sum(case when elec_name in ('elec_420009') then sum_elec else 0 end) as sum_elec5,
									sum(case when elec_name in ('elec_420012') then sum_elec else 0 end) AS sum_elec6
									from (  
            ...
            LEFT JOIN (
                 select 	DATE_FORMAT(r.jDay, '%H') as date,  
									...
									sum(case when heat_name in ('heat_420001') then sum_heat else 0 end) as sum_heat1,   
									sum(case when heat_name in ('heat_420011') then sum_heat else 0 end ) as sum_heat2, 
									sum(case when heat_name in ('heat_420002') then sum_heat else 0 end ) as sum_heat3,
									sum(case when heat_name in ('heat_420009') then sum_heat else 0 end ) as sum_heat4,
									sum(case when heat_name in ('heat_420010') then sum_heat else 0 end ) as sum_heat5,
									sum(case when heat_name in ('heat_420012') then sum_heat else 0 end) AS sum_heat6									
									from (  
											select DATE_FORMAT(DATE_SUB(date,INTERVAL 7 HOUR ), '%Y-%m-%d %H') as jDay 
    ...
    processNewDataFormat(rows) {
        ...
        rows.forEach(row => {
			...
			rebeng6Arr.push(row.sum_rated6 > 0 ? row.sum_rated6 : null);
        });

        return { labels, rebeng1Arr, rebeng2Arr,rebeng3Arr,rebeng4Arr,rebeng5Arr,rebeng6Arr };
    }
    async fetchData() {
        try {
			debugger;
            ...
            if (!res || !res.data) return { labels: [], rebeng1Arr: [], rebeng2Arr: [], rebeng3Arr: [], rebeng4Arr: [], rebeng5Arr: [], rebeng6Arr: [] };
            ...
            const rebeng6Arr = this.mapDataToArray(res.data, '热泵6', this.state.timeAnchors.today.startObj);
            ...
            return {
                ...
                rebeng6Arr: [40, 70, 80, 90, 100, 85, 95, 110, 105, 115]
            };
        }
    }
    ...
    async updateChart() {
        ...
        try {
            const { labels, rebeng1Arr, rebeng2Arr, rebeng3Arr, rebeng4Arr, rebeng5Arr, rebeng6Arr } = await this.fetchData();
