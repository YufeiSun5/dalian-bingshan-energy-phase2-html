window.HEATPUMP_CONFIG = {
    ...
    generateSQL_ByEquipment: function(startDate, endDate, dateMode) {
        ...
        // 【关键】：只有按月查询日数据时才需要 7 小时偏移
        // 按日查询小时数据时不需要偏移（否则小时会错位）
        var needOffset = (dateMode === 'month');
        var dateFormatExpr = needOffset 
            ? `date_format(date_sub(date, INTERVAL 7 HOUR), '${timeFormat}')` 
            : `date_format(date, '${timeFormat}')`;
...
elec_from_original AS (
    ...
    FROM (
        SELECT 
            no, 
            ${dateFormatExpr} AS query_date_hour, 
            MIN(KWH) as min_val, 
            MAX(KWH) as max_val
        FROM data_all 
        WHERE 
...
-- 1.3: 合并两部分数据
elec_combined AS (
    SELECT no, query_date, hourly_usage FROM elec_from_summary
    UNION ALL
    SELECT no, query_date, hourly_usage FROM elec_from_original
),
    ...
    SELECT 
        ...
        CASE 
            WHEN cfg.NO = 420010 THEN 
                 GREATEST(0, COALESCE((SELECT hourly_usage FROM elec_combined WHERE no = 320022 AND query_date = ec.query_date), 0) - 
                             COALESCE((SELECT hourly_usage FROM elec_combined WHERE no = 420001 AND query_date = ec.query_date), 0) -
							 COALESCE((SELECT hourly_usage FROM elec_combined WHERE no = 420002 AND query_date = ec.query_date), 0) -
							 COALESCE((SELECT hourly_usage FROM elec_combined WHERE no = 420009 AND query_date = ec.query_date), 0)
						)
            ELSE COALESCE(ec.hourly_usage, 0)
        END AS elec_val
    FROM info_config cfg
    JOIN elec_combined ec ON cfg.NO = ec.no OR (cfg.NO = 420010 AND ec.no = 320022)
