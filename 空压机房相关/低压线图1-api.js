...
class ChartDataManager {
    ...
    generateSQL() {
        const sql = `
            SELECT
                ...
                -- 2. 计算平均值
                ROUND(AVG(p.pressure), 1) AS system_avg_pressure
            FROM
                sys_ints t
            LEFT JOIN
                air_compressor_pressure p
                ON p.date >= (DATE(NOW() - INTERVAL 7 HOUR) + INTERVAL 7 HOUR) + INTERVAL t.i HOUR
               AND p.date <  (DATE(NOW() - INTERVAL 7 HOUR) + INTERVAL 7 HOUR) + INTERVAL (t.i + 1) HOUR
               AND p.no IN (410008) -- 筛选必须放在 ON 里，否则空时间段会被过滤掉
            WHERE
                -- 3. 取出 0 到 24，共 25 个小时点 (从今天 7:00 到明天 7:00)
                t.i <= 24
            GROUP BY
                hour_period
            ORDER BY
    ...
    async fetchData() {
        try {
            ...
            const values = response.data.map(item => item.system_avg_pressure);
