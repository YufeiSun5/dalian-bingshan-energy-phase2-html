...
window.RealtimeModule = {
    ...
    exposeControlMethods: function() {
        ...
        window.CompressorControl = {
            ...
            setTankFrontPressure: (value) => {
                const formattedValue = window.formatNumericValue(value, 2);
                module.dataPointsConfig.tankFrontPressure.value = formattedValue;
                const element = document.getElementById('tank-front-pressure');
                if (element) element.textContent = formattedValue;
                //console.log(`✓ 已设置大罐前压力: ${formattedValue}`);
            },
            
            // 设置大罐压力
            setTankPressure: (value) => {
                const formattedValue = window.formatNumericValue(value, 2);
                module.dataPointsConfig.tankPressure.value = formattedValue;
                const element = document.getElementById('tank-pressure');
                if (element) element.textContent = formattedValue;
                //console.log(`✓ 已设置大罐压力: ${formattedValue}`);
            },
            
            // 设置出口压力
            setOutletPressure: (value) => {
                const formattedValue = window.formatNumericValue(value, 2);
                module.dataPointsConfig.outletPressure.value = formattedValue;
                const element = document.getElementById('outlet-pressure');
                if (element) element.textContent = formattedValue;
                //console.log(`✓ 已设置出口压力: ${formattedValue}`);
            },
            
            // 设置出口露点
            setOutletDewpoint: (value) => {
                const formattedValue = window.formatNumericValue(value, 1);
                module.dataPointsConfig.outletDewpoint.value = formattedValue;
                const element = document.getElementById('outlet-dewpoint');
                if (element) element.textContent = formattedValue;
                //console.log(`✓ 已设置出口露点: ${formattedValue}`);
            },
            
            // 设置出流量
            setOutletFlow: (value) => {
                const formattedValue = window.formatNumericValue(value, 1);
                module.dataPointsConfig.outletFlow.value = formattedValue;
                const element = document.getElementById('outlet-flow');
                if (element) element.textContent = formattedValue;
                //console.log(`✓ 已设置出流量: ${formattedValue}`);
            },
    ...
    /**
     * 设置冷干机电量
     * @param {Number|String} value - 电量数值 (kW)
     */
    setDryerPower: function(value) {
        ...
        this.dryerConfig.data.power = formattedValue;
    },
    ...
    /**
     * 设置今日总用量
     * @param {Number|String} value - 用量数值 (KWh)
     */
...
window.HighPressureModule = {
     ...
     * 更新活塞机运行时长
     * @param {Number} pistonIndex - 活塞机编号 (1-4)
     * @param {Number|String} hours - 运行时长（小时）
     */
    updatePistonHours: function(pistonIndex, hours) {
        ...
        const formattedHours = window.formatNumericValue(hours, 0);
    ...
    updateSystemParams: function(params) {
        // 更新联锁状态指示灯
        if (params.interlockStart !== undefined) {
            this.updateInterlockStatus('start', params.interlockStart);
        }
        if (params.interlockStop !== undefined) {
            this.updateInterlockStatus('stop', params.interlockStop);
        }
        
        // 更新系统压力
        if (params.systemPressure !== undefined) {
            const element = document.getElementById('system-pressure');
            if (element) element.textContent = window.formatNumericValue(params.systemPressure, 2);
        }
        
        // 更新瞬时流量
        if (params.instantFlow !== undefined) {
            const element = document.getElementById('instant-flow');
            if (element) element.textContent = window.formatNumericValue(params.instantFlow, 1);
        }
        
        // 更新累计流量
        if (params.accumulatedFlow !== undefined) {
            const element = document.getElementById('accumulated-flow');
            if (element) element.textContent = window.formatNumericValue(params.accumulatedFlow, 1);
        }
        ...
        if (params.voltages) {
            if (params.voltages.uab !== undefined) {
                ...
                if (element) element.textContent = window.formatNumericValue(params.voltages.uab, 1);
            }
            if (params.voltages.ubc !== undefined) {
                const element = document.getElementById('voltage-ubc');
                if (element) element.textContent = window.formatNumericValue(params.voltages.ubc, 1);
            }
            if (params.voltages.uca !== undefined) {
                const element = document.getElementById('voltage-uca');
                if (element) element.textContent = window.formatNumericValue(params.voltages.uca, 1);
            }
        }
        ...
        if (params.currents) {
            if (params.currents.ia !== undefined) {
                ...
                if (element) element.textContent = window.formatNumericValue(params.currents.ia, 1);
            }
            if (params.currents.ib !== undefined) {
                const element = document.getElementById('current-ib');
                if (element) element.textContent = window.formatNumericValue(params.currents.ib, 1);
            }
            if (params.currents.ic !== undefined) {
                const element = document.getElementById('current-ic');
                if (element) element.textContent = window.formatNumericValue(params.currents.ic, 1);
            }
        }
        ...
        if (params.activeEnergy !== undefined) {
            ...
            if (element) element.textContent = window.formatNumericValue(params.activeEnergy, 1);
        }
    },
    
    /**
     * 更新联锁状态指示框
     * @param {String} type - 类型: 'start' (启动) 或 'stop' (停止)
     * @param {Number} value - 状态值: 0 (未激活) 或 1 (激活)
     */
    updateInterlockStatus: function(type, value) {
        const elementId = type === 'start' ? 'interlock-start-box' : 'interlock-stop-box';
        const box = document.getElementById(elementId);
        if (!box) return;
        
        // 移除所有激活状态类
        box.classList.remove('active-start', 'active-stop');
        
        // 根据值和类型添加对应的激活状态类
        if (value === 1 || value === '1' || value === true) {
            if (type === 'start') {
                box.classList.add('active-start');
            } else {
                box.classList.add('active-stop');
            }
        }
    },
...
window.CompressorSystemUpdate = {
    ...
    updateLowPressure: function(data) {
        ...
        if (data.compressors && Array.isArray(data.compressors)) {
            if (window.RealtimeModule && window.RealtimeModule.compressorConfig) {
                ...
                data.compressors.forEach((comp, index) => {
                    ...
                    // 合并新数据（只更新提供的字段）
                    // 使用formatValue处理数值，支持字符串和数字两种格式
        ...
        if (data.chartData && Array.isArray(data.chartData)) {
            if (window.CompressorControl) {
                ...
                // setChartData 已经有//console.log，这里不重复输出
            } else {
        ...
        if (data.usage) {
            if (window.CompressorControl) {
                ...
                // setTodayUsage 和 setYesterdayUsage 已经有//console.log
            } else {
        ...
        if (data.dryer) {
            if (window.CompressorControl) {
                ...
                // setDryerPower 和 setDryerStatus 已经有//console.log
            } else {
    ...
    updateUnitController: function(data) {
        ...
        if (data.systemPressure !== undefined && window.UnitControllerControl) {
            ...
            // setSystemPressure 已经有//console.log
        }
        ...
        if (data.pressure !== undefined && window.UnitControllerControl) {
            ...
            // setPressure 已经有//console.log
        }
        
        // 更新压力参数（这些参数通常是字符串，保持原样）
        if (data.pressureParams && window.UnitControllerControl) {
            ...
            // setPressureParams 已经有//console.log
        }
    },
    ...
    help: function() {
        ...
        //console.log('%c  • 支持字符串和数字两种格式，数字会自动格式化小数位数', 'color: #cccccc;');
        
        //console.log('\n%c数据格式支持：', 'color: #ffaa00;');
        //console.log('%c  • 字符串格式：直接显示，如 "8.5" 或 "125"', 'color: #cccccc;');
        //console.log('%c  • 数字格式：自动格式化小数位，如 8.5 → "8.50", 125.123456 → "125.1"', 'color: #cccccc;');
        //console.log('%c  • 压力值：保留2位小数', 'color: #cccccc;');
        //console.log('%c  • 电流值：保留1位小数', 'color: #cccccc;');
        //console.log('%c  • 用量值：保留0位小数（整数）', 'color: #cccccc;');
